...
const setConsNode = async function (nodeInfo, socket, nodeRoleInt) {
    ...
    await dbUtil.queryPre(
        dbIS.querys.node_hw_info.insert_node_hw_info, [
    ...
    await dbUtil.queryPre(
        dbIS.querys.node_cons_info.insert_node_cons_info, [
            define.P2P_DEFINE.P2P_SUBNET_ID_IS, 
            remoteIpStr, //ip
            nodeInfo.myHwInfo.mInfo.p2pAddr, //p2paddr
            nodeRoleInt, //role (Default Setting : CN)
            define.STATE.OFF, //state (Default Setting : OFF)
            nodeInfo.myHwInfo.mInfo.kafkaIdx, 
            nodeInfo.myHwInfo.mInfo.hubCode, 
            nodeInfo.myHwInfo.pubkey, //public_key
            nodeInfo.myHwInfo.w_pubkey //wallet_public_key
            // nodeInfo.myHwInfo.mCalc.totalPrrPoint, // total_prr
            // nodeInfo.myHwInfo.id, //id
        ]
    );
...
const getRoleInt = (roleStr) => {
    ...
    {
        roleInt = define.NODE_ROLE.NUM.CN;
    }
...
const updateNodeInfo = async function (nodeInfo, nodeRoleInt, total_prr_point) {
    ...
    if ((nodeRoleInt === define.NODE_ROLE.NUM.NN) || (nodeRoleInt === define.NODE_ROLE.NUM.ISAG))
    {
        ...
        let nodeClusterInfoRes = await dbUtil.queryPre(dbIS.querys.cluster_info.cluster_p2p_addr, [nodeInfo.myHwInfo.snHash]);
        ...
        let hubGps = await dbUtil.query(dbIS.querys.hub_info.gps_code_hub);
        if (hubGps.length)
        {
...
const getKafkaIdx = async (p2pAddr) => {
    ...
    kafkaInfo = await dbUtil.queryPre(dbIS.querys.kafka_info.kafka_info_w, [p2pAddr]);
    if (kafkaInfo.length)
    {
        // for (var i = 0; i < kafkaInfo.length; i++)
        // {
        //     if (((kafkaInfo[i].topic_list).split(',')).indexOf(p2pAddr) !== define.ERR_CODE.ERROR)
        //     {
...
const gpsToP2pAddr = (latitude, longitude) => {
    let lat_split = latitude.split('.');
    let lon_split = longitude.split('.');
    let gps_p2p_addr = '0x'
