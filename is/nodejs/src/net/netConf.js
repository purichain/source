...
const rrNetJsonMaker = async function(list){
    ...
    //for test
    let revision = await dbUtil.query(dbIS.querys.revision.reset_count);
    ...
    net_tier.GEN_INTERVAL = config.netConfSet.genInterval;
    net_tier.GEN_ROUND_CNT = config.netConfSet.genRountCnt;
    ...
    net.TIER_NUM = config.netConfSet.tierNum;
...
const nodeJsonMaker = async function(kind, list, count, rrNet){
    ...
    if(kind === define.NODE_ROLE.STR.NN)
    {
        ...
        cluster.MAX = config.netConfSet.max;
        p2p.CLUSTER = cluster;
        node.P2P = p2p;

        // NODE : SOCK : NUM
        num.UDP_SVR = config.netConfSet.udpSvr;
        num.UDP_CLI = config.netConfSet.udpCli;
        num.TCP_SVR = config.netConfSet.nn_tcpSvr;
        if(nn_cnt === 1)
        {
            num.TCP_CLI = config.netConfSet.tcpCli_0;
        }
        else if(nn_cnt > 1)
        {
            num.TCP_CLI = config.netConfSet.tcpCli;
        }
        else
        {
            // Error
        ...
        if (rrNet.NET.TIER[config.netConfSet.tierNum - 1].NN_LIST.length === 1)
        {
        ...
        if (rrNet.NET.TIER[config.netConfSet.tierNum - 1].NN_LIST.length > 1)
         {
            ...
            peers2 = {
                IP: rrNet.NET.TIER[config.netConfSet.tierNum - 1].NN_LIST[next_rr_idx].SOCK.IP,
        ...
        // NODE : SOCK : TCP_CLI_1
        tcp_cli_1.AUTO_JOIN = config.netConfSet.nn_autoJoin;
        tcp_cli_1.P2P_JOIN = config.netConfSet.nn_p2pJoin;
        ...
        if(rrNet.NET.TIER[config.netConfSet.tierNum-1].NN_LIST.length > 1)
        {
            ...
            let tcp_cli_peer = {
                IP: rrNet.NET.TIER[config.netConfSet.tierNum - 1].NN_LIST[next_rr_idx].SOCK.IP,
                PORT: rrNet.NET.TIER[config.netConfSet.tierNum - 1].NN_LIST[next_rr_idx].SOCK.PORT
            };
            let tcp_cli_local = {
...
module.exports.makeNetConf = async (count) => {
    ...
    let nnList = await dbUtil.queryPre(dbIS.querys.node_cons_info.get_node_list_role, [define.NODE_ROLE.NUM.NN]);
    if (nnList.length)
    {
        // rr_net.json
        ...
        await dbUtil.queryPre(dbIS.querys.revision.net_reset, [define.P2P_DEFINE.P2P_SUBNET_ID_IS, await util.timeStampMS(), JSON.stringify(rrNetJson)]);
