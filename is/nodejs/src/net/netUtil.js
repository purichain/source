...
module.exports.getTopicName = async function (p2paddr) {
    let topic;
    let idcInfo = await JSON.parse(await dbUtil.query(dbIS.querys.hub_info.total_hub_list));
    ...
    let designatedIDC = await dbUtil.queryPre(dbIS.querys.hub_info.designated_idc, [min[0].Name]);
...
module.exports.nearestIDC = async (ip) => {
    let idc_gps = await JSON.parse(await dbUtil.query(dbIS.querys.hub_info.designated_idc2));
