...
const deleteDbTableISA = async (closeIP) => {
    ...
    else
    {
        ...
        await dbUtil.queryPre(dbIS.querys.node_cons_info.delete_node_cons_info, [closeIP]);
        logger.info("[TCP] [ISA] Delete node_cons_info Success");

        await dbUtil.queryPre(dbIS.querys.node_hw_info.delete_node_hw_info, [closeIP]);
...
module.exports.bindISAServer = async function(){
    ...
    is.listen(config.SOCKET_INFO.BIND_ISA_PORT, async function () {
...
module.exports.bindISAgServer = async function(){
    ...
    is.listen(config.SOCKET_INFO.BIND_ISAG_PORT, async function () {
...
async function stopBlockGen(Res){
    Res = await cliHandler.blockGenStop();
    if (Res) {
